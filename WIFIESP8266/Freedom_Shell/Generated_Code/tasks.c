...
PRIVILEGED_DATA static portTickType xNextTaskUnblockTime                                                = ( portTickType ) portMAX_DELAY;
        ...
        static void prvGenerateRunTimeStatsForTasksInList(unsigned char *pcWriteBuffer, size_t bufSize, xList *pxList, unsigned long ulTotalRunTime ) PRIVILEGED_FUNCTION;
 ...
 */
        ...
        static void prvListTaskWithinSingleList(unsigned char *pcWriteBuffer, size_t bufSize, xList *pxList, signed char cStatus ) PRIVILEGED_FUNCTION;
...
/*-----------------------------------------------------------*/
        ...
        unsigned portBASE_TYPE uxQueue;
                ...
                vTaskSuspendAll();
                {
                  ...
                  UTIL1_strcpy(pcWriteBuffer, bufSize, (const unsigned char *) "\r\n" );
                        ...
                        do
                        {
                                ...
                                {
                                        prvListTaskWithinSingleList( pcWriteBuffer, bufSize, ( xList * ) &( pxReadyTasksLists[ uxQueue ] ), tskREADY_CHAR );
                                }
                        }while( uxQueue > ( unsigned short ) tskIDLE_PRIORITY );

                        if( listLIST_IS_EMPTY( pxDelayedTaskList ) == pdFALSE )
                        {
                                prvListTaskWithinSingleList( pcWriteBuffer, bufSize, ( xList * ) pxDelayedTaskList, tskBLOCKED_CHAR );
                        }

                        if( listLIST_IS_EMPTY( pxOverflowDelayedTaskList ) == pdFALSE )
                        {
                                prvListTaskWithinSingleList( pcWriteBuffer, bufSize, ( xList * ) pxOverflowDelayedTaskList, tskBLOCKED_CHAR );
                        }
                        ...
                        #if( INCLUDE_vTaskDelete == 1 )
                        {
                                ...
                                {
                                        prvListTaskWithinSingleList( pcWriteBuffer, bufSize, &xTasksWaitingTermination, tskDELETED_CHAR );
                                }
                        }
                        #endif
                        ...
                        #if ( INCLUDE_vTaskSuspend == 1 )
                        {
                                ...
                                {
                                        prvListTaskWithinSingleList( pcWriteBuffer, bufSize, &xSuspendedTaskList, tskSUSPENDED_CHAR );
                                }
                        }
                        #endif
                }
...
/*----------------------------------------------------------*/
  ...
  void vTaskGetRunTimeStats(unsigned char *pcWriteBuffer, size_t bufSize)
      ...
      UTIL1_strcpy(pcWriteBuffer, bufSize, (const unsigned char*)"\r\n");
        ...
        if( listLIST_IS_EMPTY( &( pxReadyTasksLists[ uxQueue ] ) ) == pdFALSE )
        {
          prvGenerateRunTimeStatsForTasksInList( pcWriteBuffer, bufSize, ( xList * ) &( pxReadyTasksLists[ uxQueue ] ), ulTotalRunTime );
        }
      }while( uxQueue > ( unsigned short ) tskIDLE_PRIORITY );

      if( listLIST_IS_EMPTY( pxDelayedTaskList ) == pdFALSE )
      {
        prvGenerateRunTimeStatsForTasksInList( pcWriteBuffer, bufSize, ( xList * ) pxDelayedTaskList, ulTotalRunTime );
      }

      if( listLIST_IS_EMPTY( pxOverflowDelayedTaskList ) == pdFALSE )
      {
        prvGenerateRunTimeStatsForTasksInList( pcWriteBuffer, bufSize, ( xList * ) pxOverflowDelayedTaskList, ulTotalRunTime );
      }
      ...
      #if ( INCLUDE_vTaskDelete == 1 )
      {
        if( listLIST_IS_EMPTY( &xTasksWaitingTermination ) == pdFALSE )
        {
          prvGenerateRunTimeStatsForTasksInList( pcWriteBuffer, bufSize, &xTasksWaitingTermination, ulTotalRunTime );
        }
      }
      #endif
      ...
      #if ( INCLUDE_vTaskSuspend == 1 )
      {
        if( listLIST_IS_EMPTY( &xSuspendedTaskList ) == pdFALSE )
        {
          prvGenerateRunTimeStatsForTasksInList( pcWriteBuffer, bufSize, &xSuspendedTaskList, ulTotalRunTime );
        }
      }
      #endif
    }
...
static void prvInitialiseTCBVariables( tskTCB *pxTCB, const signed char * const pcName, unsigned portBASE_TYPE uxPriority, const xMemoryRegion * const xRegions, unsigned short usStackDepth )
{
        ...
        #if configMAX_TASK_NAME_LEN > 1
        {
                ...
                (void)strncpy( ( char * ) pxTCB->pcTaskName, ( const char * ) pcName, ( unsigned short ) configMAX_TASK_NAME_LEN );
        }
        #endif
...
/*-----------------------------------------------------------*/
...
#if ( configUSE_TRACE_FACILITY == 1 )
        static void prvListTaskWithinSingleList(unsigned char *pcWriteBuffer, size_t bufSize, xList *pxList, signed char cStatus )
        {
        ...
        /* PRIVILEGED_DATA static char pcStatusString[ configMAX_TASK_NAME_LEN + 30 ]; */
                ...
                do
                {
                        ...
                        #endif          	
      UTIL1_strcat(pcWriteBuffer, bufSize, (const unsigned char*)pxNextTCB->pcTaskName);
      ...
      UTIL1_strcatNum32u((unsigned char*)pcWriteBuffer, bufSize, pxNextTCB->uxPriority);
      UTIL1_chcat(pcWriteBuffer, bufSize, (unsigned char)'\t');
      UTIL1_strcatNum32u(pcWriteBuffer, bufSize, usStackRemaining);
      UTIL1_chcat(pcWriteBuffer, bufSize, (unsigned char)'\t');
      UTIL1_strcatNum32u(pcWriteBuffer, bufSize, pxNextTCB->uxTCBNumber);
      UTIL1_strcat(pcWriteBuffer, bufSize, (const unsigned char*)"\r\n");
...
#if ( configGENERATE_RUN_TIME_STATS == 1 )
  static void prvGenerateRunTimeStatsForTasksInList(unsigned char *pcWriteBuffer, size_t bufSize, xList *pxList, unsigned long ulTotalRunTime )
  {
  volatile tskTCB *pxNextTCB, *pxFirstTCB;
    ...
    do
    {
      ...
      if( ulTotalRunTime > 0UL )
      {
        ...
        if( pxNextTCB->ulRunTimeCounter == 0UL )
        {
          ...
          UTIL1_strcat(pcWriteBuffer, bufSize, (const unsigned char*)pxNextTCB->pcTaskName);
          UTIL1_strcat(pcWriteBuffer, bufSize, (const unsigned char*)"\t\t0\t\t0%\r\n");
        }
        else
        {
          ...
          if( ulStatsAsPercentage > 0UL )
          {
            UTIL1_strcat(pcWriteBuffer, bufSize, (const unsigned char*)pxNextTCB->pcTaskName);
            UTIL1_strcat(pcWriteBuffer, bufSize, (const unsigned char*)"\t\t");
            UTIL1_strcatNum32u(pcWriteBuffer, bufSize, pxNextTCB->ulRunTimeCounter);
            UTIL1_strcat(pcWriteBuffer, bufSize, (const unsigned char*)"\t\t");
